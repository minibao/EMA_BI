...
public class JedisUtils {
    protected Logger log = LoggerFactory.getLogger(JedisUtils.class);
    ...
    public Jedis getJedis() {
        ...
        do {
            try {
                jedis = getPool(RedisConfig.HOST, RedisConfig.PORT).getResource();
                log.debug("get redis master1!");
            } catch (Exception e) {
                log.error("get redis master1 failed!", e);
                getPool(RedisConfig.HOST, RedisConfig.PORT).returnBrokenResource(jedis);
            }
            count++;
    ...
    public void closeJedis(Jedis jedis) {
        if (jedis != null) {
            getPool(RedisConfig.HOST, RedisConfig.PORT).returnResource(jedis);
        }
    }
