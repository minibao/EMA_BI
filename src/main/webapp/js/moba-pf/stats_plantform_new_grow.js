                    ...
                    _.each(plantformNewgrow, function (item, index) {
                        ...
                        if (tagArr.length <= 1) {
                            ...
                            if (_data == '' || _data == item.regDate) {
                                gdevicec = Number(gdevicec) + Number(item.gameDeviceCount);
                                guidc = Number(guidc) + Number(item.gameUidCount);
                                pdevicec = Number(pdevicec) + Number(item.pfDeviceCount);
                                puidc = Number(puidc) + Number(item.pfUidCount);
                                _data = item.regDate;
                            } else {
                                e_data_gameDC.push([_data, gdevicec]);
                                e_data_gameUC.push([_data, guidc]);
                                e_data_pfDC.push([_data, pdevicec]);
                                e_data_pfUC.push([_data, puidc]);
                                _data = item.regDate;
                                gdevicec = Number(item.gameDeviceCount);
                                guidc = Number(item.gameUidCount);
                                pdevicec = Number(item.pfDeviceCount);
                                puidc = Number(item.pfUidCount);
                            }
